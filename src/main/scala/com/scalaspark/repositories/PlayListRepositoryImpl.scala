...
class PlayListRepositoryImpl(sparkServices: SparkServices, dataBase: String, table: String, format: String) extends PlayListRepository {
  ...
  override def findById(id: Long): PlayList = {
    val spark = sparkServices.connect
    import spark.implicits._
    val playList = spark.sql("select " + PlayListEnums.ID.toString + ", " + PlayListEnums.NAME.toString + " from " + dataBase.concat(table) + " where " + PlayListEnums.ID + " = " + id).dropDuplicates.as(Encoders.bean(classOf[PlayList])).first
    playList
  }
  ...
  override def findAllPlayList(): util.List[String] = {
    val spark = sparkServices.connect
    import spark.implicits._
    val names = spark.sql("select " + PlayListEnums.NAME + " from " + dataBase.concat(table)).dropDuplicates.as(Encoders.bean(classOf[String])).collectAsList()
    names
  }
}
